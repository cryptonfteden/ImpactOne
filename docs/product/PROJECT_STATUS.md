...
## 24. Sprint 16 Phase D - Decision Quality and Explainability

Scope note: builds a decision-quality layer on top of the existing recommendation engine — no new data sources, no execution. Almost entirely an aggregation exercise: `processEvent` (`autonomousMarketService.js`) already computed `impactType`, `riskLevel`, `counterarguments`, and `invalidationSignals` per matched event, and `scenarioEngineService.getScenario()` already existed with real bull/base/bear narratives/probabilities — both previously unused by the recommendation engine. Phase D wires them in rather than inventing new analysis.

Sprint 16 Phase D outcomes:
- **Structured explanation** (`Recommendation.explanation`, JSON) — `thesis`, `supportingEvidence`/`opposingEvidence` (matched events split by whether their already-computed `impactType` aligns with or opposes the recommended action; opposing entries also carry their own `counterarguments`), `keyRisks` (high-`riskLevel` opposing events + concentration/macro factors), `invalidationConditions` (deduped `invalidationSignals` across matched events), `timeHorizon`, `affectedPositions`/`affectedWatchlistSymbols` (0-1 entries, symbol-scoped — not cross-symbol correlation, per the Phase C planning precedent), and `confidenceDrivers`/`confidenceReducers` (built from the ranking sub-scores — opportunity/risk/momentum/institutional/prediction-market/macro-exposure — already computed since Sprint 12/16A but never surfaced before).
- **Bull/base/bear scenarios** (`Recommendation.scenarios`, JSON array of 3) — reuses `scenarioEngineService.getScenario(primaryDriver)` for theme-matched narratives/probabilities/sector rotation, layered with catalysts/risks from the same supporting/opposing split, and price impact from the existing conviction-tiered `buildPortfolioAction` (offset ±15, not a new formula). `portfolioImpact` is a labeled approximation (position weight % × price-range midpoint) that returns `null` rather than guessing when the range text has no extractable number (e.g. "Wait for confirmation").
- **Transparent quality score** (`Recommendation.qualityScore` + `qualityComponents`, both new columns) — six documented, weighted (15/15/20/20/10/20%) 0-100 components: source quality and evidence freshness reuse Phase C's `sourceQualityScore`/`recencyScore` article-ranking functions; portfolio relevance, evidence agreement, data completeness, and model confidence are new, each traced to real data (symbolSource/position weight, the supporting-vs-opposing ratio, a 4-point completeness checklist, and convictionScore directly). Exposed as both the rollup and the six named components, per the "expose the score and its components" requirement.
- **Immutable decision trace** — new `DecisionTrace` model, 1:1 with `Recommendation`, cascade-deleted with it. The repository (`autonomousRecommendationRepository.js`) exposes only `createDecisionTrace`/`getDecisionTraceByRecommendationId` — no update method exists anywhere, verified by a test asserting no such export exists. Contains only already-processed application data (ranking scores, matched-event summaries, portfolio snapshot) — a dedicated test asserts the serialized trace never contains the configured `FINNHUB_API_KEY`/`NEWS_API_KEY`/`OPENAI_API_KEY` values. New `GET /api/v2/recommendations/:id/decision-trace` endpoint, kept separate from the main detail response.
- **Frontend** — `RecommendationCard.jsx` (new, extracted from `RecommendationsScreen.jsx` as the card roughly tripled in content) plus `ScenarioComparison.jsx` (bull/base/bear 3-column grid) and `QualityScoreBreakdown.jsx` (the six named components). Card always shows the provenance badge, a new color-tiered quality-score badge, and the explanation's `thesis` as the primary summary; expanded view adds exact portfolio exposure, supporting/opposing evidence with counterarguments, key risks, invalidation conditions, confidence drivers/reducers, the scenario comparison, quality breakdown, and matched-event citations now including a `publishedAt` timestamp alongside the existing source name/link/confidence. `RecommendationsPreview.jsx` (Dashboard) gets the quality badge only, staying a lightweight preview.
- **Documentation gap closed** — `API_CONTRACTS.md` had never documented any `/api/v2/recommendations/*` endpoint since Phase A shipped it; §3.39-3.43 now cover list/detail/run/status/decision-trace with the full Phase D response schema.

Sprint 16 Phase D verification:
- `npm run build` — frontend build passed.
- `npm run test` — 135/135 passing at completion (89 backend, 46 frontend; +49 net new tests this phase), run before each of the 4 commits.
- Direct unit tests for `computeQualityScore` (all six components + weighted rollup, edge cases with no matched events) and decision-trace creation/retrieval/cascade-delete, per requirement #7.
- Manual end-to-end run: `POST /api/v2/recommendations/run` against the real (unmocked) pipeline produced a recommendation with a populated `explanation` (real thesis/evidence/risks/invalidation/confidence factors), 3 real scenarios with real catalysts drawn from actual matched events, and a real quality-score breakdown; `GET /:id/decision-trace` returned the full trace with no secret values present; `/api/v2/portfolio/trades` stayed empty before and after, confirming the advisory-only invariant still holds — the engine still never imports `placeOrder`.
- Browser verification (Playwright, headless Chromium): both the Recommendations screen and Dashboard preview render the quality badge; expanding a card shows the bull/base/bear comparison, quality breakdown, invalidation conditions, and supporting evidence sections; no new console errors (the pre-existing Sprint 15 `MarketContextStrip` 404s are untouched).

Not in scope for Sprint 16 Phase D:
- Everything already excluded from Phase A/B/C (execution/broker connectivity, calibration loop, auth/billing, persisted server-side watchlist, cross-symbol correlation) — unchanged.
- A UI for the decision trace — requirement #5's UI bullet list (understand/expand evidence/compare scenarios/invalidation/exposure/citations) doesn't ask for one; it's exposed via API as an audit artifact for now.

## 25. Sprint 18A - Canonical Decision Architecture

Scope note: implements the architectural corrections `INTELLIGENCE_PLATFORM_REVIEW.md` (an independent architecture review) found necessary before building the Research Intelligence Engine — specifically its §10 items 1, 2, 3, and 7: a single canonical verdict contract, one shared scoring vocabulary, a frozen canonical Event Envelope, and structurally (not just conventionally) enforced `DecisionTrace` immutability. The review's core finding: the Investment Committee (`Strong Buy…Strong Sell`) and the Recommendation Engine (`BUY/REDUCE/EXIT`) independently computed two verdicts that could disagree in front of the same user on the same symbol — confirmed directly in code (`aiController.js` called both and embedded both; `AiAnalysisScreen.jsx` rendered both pills in separate sections of the same page). Deliberately scoped conservatively: the Committee's own alt-data/intelligence fan-out (review §2/§5) is preserved as-is — deduplicating that is a separate, later initiative, not this sprint's objective.

Sprint 18A outcomes:
- **Canonical verdict contract** (`backend/services/canonicalVerdict.js`) — `buildCanonicalVerdictView` is the one function that assembles what an API response exposes: exactly one `action` field, sourced only from a persisted `Recommendation`. A structural guard (`sanitizeCommitteeDebate`) strips any `action`/`decision`/`verdict`/`finalDecision`/`recommendation`-shaped key from committee output before it can reach a response, independent of whether `investmentCommitteeService.js` itself ever produces one.
- **Investment Committee folded into a debate layer** — `analyzeInvestmentCommittee` keeps its five-agent debate (bull/bear arguments, votes, confidence) and CIO synthesis internally unchanged, but no longer calls `committeeTrackRecordService.upsertCommitteeDecision` (removing a synchronous, unlocked JSON-file write flagged as a bottleneck) and no longer returns an independent `cio.decision`. Its response is now `committeeDebate`: `supportingArguments`/`opposingArguments` (each argument tagged by originating agent), `expertVotes` (raw per-agent votes — explicitly allowed as individual expert opinion, never a synthesized aggregate), `disagreementLevel`/`consensusLevel`, `specialistObservations`, and a `synthesis` narrative with `decision` stripped. `committeeTrackRecordService.js`'s historical JSON data remains fully readable as a labeled legacy view — nothing was deleted or migrated, only frozen.
- **Shared scoring vocabulary** (`backend/services/scoringVocabulary.js`) — one documented range/meaning/formula/fallback contract for `confidence`, `conviction`, `quality`, `risk`, `relevance`, `sourceCredibility`, `evidenceFreshness`, `evidenceAgreement`, and a genuinely new `uncertainty` score (derived from evidence agreement + committee consensus, not simply `100 - confidence`). Wraps existing, already-tested scorers (`autonomousMarketService.sourceQualityScore`/`recencyScore`) rather than duplicating them. Full detail: `API_CONTRACTS.md` §3.44.
- **Canonical Event Envelope** (`backend/services/eventEnvelope.js`) — the 19-field schema from the requirements, frozen ahead of the Research Intelligence Engine build. `adaptLegacyFeedItemToEnvelope` proves the schema against the one real event source that exists today, reusing `classifyEventType`/scoring-vocabulary functions rather than new analysis. Stored in `DecisionTrace.evidenceReferences` **alongside**, not instead of, the pre-existing `matchedEvents` shape other code/tests/UI already depend on. Full detail: `API_CONTRACTS.md` §3.45.
- **`DecisionTrace` gains three additive, nullable columns** — `committeeDebate`, `evidenceReferences`, `modelVersionMetadata` — via one migration (`20260712143057_add_canonical_decision_fields`). Historical rows are valid and readable unchanged; immutability (create + read only, no update method anywhere in the repository) is untouched. `confidenceCalculation` is enriched in code (no schema change) with `conviction` and `uncertainty`.
- **Recommendation Engine integration** — `evaluateSymbol()` runs the committee's debate layer only for symbols where an action already triggered (same gate as recommendation creation itself — bounded cost, and already off the request path since `runOnce()` is a scheduled background job). A committee-call failure degrades to no debate, never a broken run. The debate is threaded into both `Recommendation.explanation.committeeDebate` (for direct UI consumption without a second fetch) and `DecisionTrace.committeeDebate` (immutable audit copy).
- **Frontend** — `AiAnalysisScreen.jsx`'s Investment Committee panel no longer renders its own decision pill; it shows consensus/disagreement percentages, the synthesis narrative, and per-agent expert votes/arguments as debate context. `RecommendationCard.jsx` gains a "Committee debate" section. The legacy `useVirtualPortfolio.js` auto-trading hook (feature-flagged off by default) was updated to compute its own majority-vote heuristic from the still-published raw `expertVotes`, preserving its original threshold behavior without depending on the now-removed `cio.decision`.

Sprint 18A verification:
- `npm run test` — 125 backend / 50 frontend passing at completion (+53 net new backend tests, +4 net new frontend tests), run before each of the 9 commits.
- Dedicated integration test (`backend/routes/committee.integration.test.js`) constructs a deliberately bearish committee debate against a persisted `BUY` recommendation and asserts the API surfaces exactly one canonical action and that the committee's raw votes never appear as an `action`/`decision`/`verdict` field anywhere in the payload — the direct regression guard for "no conflicting verdicts."
- API-compatibility test asserting every pre-Sprint-18A field on `GET /api/v2/recommendations/:id` remains present.
- Manual: `POST /api/v2/recommendations/run` against the real pipeline produced a `DecisionTrace` with populated `committeeDebate`/`evidenceReferences`/`modelVersionMetadata`, with `inputEvidence.matchedEvents` unchanged; `POST /api/committee/analyze` confirmed debate-only (no verdict implied) for a symbol with no active recommendation, and `relatedRecommendation`-attached for one that has an active recommendation; `/api/v2/portfolio/trades` stayed empty, confirming the advisory-only invariant still holds.
- Browser verification: see the Sprint 18A entry in this file's manual QA log / commit history for the live-browser pass across `AiAnalysisScreen` and `RecommendationsScreen`.

Not in scope for Sprint 18A:
- Deduplicating the Committee's own alt-data/intelligence fan-out against the Recommendation Engine's evidence gathering (review §2/§5) — explicitly deferred, a separate future initiative.
- Migrating `committeeTrackRecord.json`'s historical entries into Postgres — the file remains readable as a frozen legacy view; real ongoing outcome tracking is intended to be superseded by a future Alpha Attribution Engine (`INTELLIGENCE_PLATFORM_BLUEPRINT.md`).
- Building the Research Intelligence Engine itself (source ingestion) — this sprint only freezes the contracts (event envelope, scoring vocabulary) it will need to integrate against.
- Adding a `HOLD` value to the persisted `Recommendation.action` enum — absence of a triggered recommendation continues to mean "hold," exactly as before; a derived `HOLD` label exists only in-memory for describing committee consensus, never persisted.

## 26. Sprint 20 - My First Daily Experience

Scope note: not a features sprint but a *daily usefulness* sprint — the explicit goal was "the first version of ImpactOne I personally want to open every morning." Spans onboarding, a persisted Investor Profile, an AI-generated investment profile, a redesigned minimal Home screen, a real personalized Daily Feed, and a Theme Dashboard.

Sprint 20 outcomes:
- **Investor Profile** — a new single-tenant singleton (`InvestorProfile`, same `findFirst`/create convention as `Portfolio`), collecting age (required), country, experience level, monthly investment amount, investment goal, risk tolerance, and investment horizon. Unlike `DecisionTrace`, this is real user-editable settings data (`PATCH` supported).
- **Onboarding** — a full-screen takeover (`OnboardingFlow.jsx`), gated by whether a profile exists, shown once before any part of the existing app shell renders. Seven single-question steps, single-tap chips auto-advance, only age and horizon are non-skippable — designed and tested to complete in well under 60 seconds.
- **AI Investment Profile** (Part 2) — `generateInvestmentProfile` is entirely deterministic (documented allocation/volatility/return-assumption constants, no LLM call): a suggested stock/bond/cash allocation (age-based glide path, adjusted for risk tolerance and horizon), a diversification explanation, an expected volatility range, and experience-level-templated educational content. Every return-dependent figure carries an explicit "illustration, not a promise" disclosure. The compound-interest simulator and future-timeline chart are entirely client-side (zero network round-trips per interaction), with an adjustable assumed-return slider.
- **Home screen redesign** (Part 3) — a new default landing screen answering exactly four questions (What happened? / Why should I care? / How does it affect me? / Should I do anything today?) and nothing else, backed by one new aggregation endpoint. "Should I do anything today?" is never independently computed — it reads a real, persisted `Recommendation` through `canonicalVerdict.buildCanonicalVerdictView` (Sprint 18A), the same function that guarantees exactly one canonical action platform-wide. The existing, richer Dashboard is unchanged and still reachable from the sidebar.
- **Daily Feed and personalization** (Parts 4-5) — the previously fully-mock "Market News" screen is replaced with a real feed (`intelligenceApi.liveFeed`), each item showing headline, AI summary, importance, confidence, affected sectors/companies, time horizon, potential portfolio impact, sources, and a "Why this analysis" reasoning trace built from the event's own real `explainability` data. A new `feedPersonalizationService` layers age/risk-tolerance/horizon-derived weighting on top of the existing relevance/recency/source-quality scoring — ordering only, never fact-mutating, and deliberately adds no boost where no honest signal exists (e.g. no dividend-tagging for "passive income" goals yet).
- **Theme Dashboard** (Part 6) — 7 expandable theme pages (AI, Quantum, Defense, Energy, Space, Cyber, Healthcare — "quantum" newly added to `autonomousMarketService`'s classification/scan-universe tables; the other six already existed as real buckets). Maturity is a deterministic tier from real matching-event volume; supporting evidence is real feed events tagged by the existing `classifyEventType`; thesis text is template-generated (deliberately not a new LLM touchpoint for this first slice); confidence trend is real history from a new daily best-effort snapshot job (`ThemeConfidenceSnapshot`, mirrors `DailyBriefSnapshot`'s pattern) that starts accumulating from whenever it first runs — never backfilled with invented data.
