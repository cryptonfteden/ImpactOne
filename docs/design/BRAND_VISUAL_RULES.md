...
**Phase:** PRODUCT-IDENTITY-001. Companion to [SIGNATURE_ELEMENTS.md](../product/SIGNATURE_ELEMENTS.md). These are binding rules, not preferences — any future change (design or code) that violates one of these breaks ImpactOne's own recognizability, per the mission's "someone should recognize a screenshot instantly" bar.
