...
**Purpose:** A standing inventory of missing development tooling, in the same Mandatory / Recommended / Nice to have framing used by [IMPACTONE_RELEASE_CHECKLIST.md](../operations/IMPACTONE_RELEASE_CHECKLIST.md), so future phases can check progress against a fixed list rather than rediscovering these gaps from scratch. Every absence below was confirmed by direct repository search (config-file search plus `package.json` dependency inspection), not assumed.
