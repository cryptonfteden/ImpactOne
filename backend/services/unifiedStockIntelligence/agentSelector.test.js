...
test("TARGET_AGENT_IDS is exactly options/earnings/valuation/symbol-sentiment/insider/etf-flow/institutional/short-interest/macro/analyst-consensus, per this mission's own scope (ANALYST-CONSENSUS-AGENT-001 added the 10th)", () => {
  assert.deepEqual(TARGET_AGENT_IDS, ["options", "earnings", "valuation", "symbol-sentiment", "insider", "etf-flow", "institutional", "short-interest", "macro", "analyst-consensus"]);
});

test("selects only the 10 target agents from a real registry that has many more registered", () => {
...
test("honestly returns fewer than 10 if some target agents are missing from the registry — never fabricates a placeholder", () => {
