...
test("generateUnifiedIntelligence resolves its 9 target agents from the real, live Agent Registry — never a hardcoded import", async () => {
  const report = await generateUnifiedIntelligence("NVDA");
  assert.equal(report.totalAgentCount, 9, "options, earnings, valuation, symbol-sentiment, insider, etf-flow, institutional, short-interest, and macro must all be found in the real registry");
  assert.deepEqual(
...
test("running through a real, injected AgentExecutionLog records one real execution entry per contributing agent, all sharing the report's own correlationId", async () => {
  ...
  assert.deepEqual(records.map((r) => r.agentId).sort(), ["earnings", "etf-flow", "insider", "institutional", "macro", "options", "short-interest", "symbol-sentiment", "valuation"]);
  void log;
});

test("the report never emits a forbidden governance key (action/decision/verdict/recommendation) anywhere, even after aggregating 9 real domain reports", async () => {
  const report = await generateUnifiedIntelligence("NVDA");
