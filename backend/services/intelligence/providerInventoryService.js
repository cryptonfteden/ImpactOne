...
const KNOWN_LIVE_PROVIDER_IDS = new Set(["reutersBloombergWire", "cftcCot"]);
...
function deriveStatus({ providerId, lastStatus, configurationRequirement }) {
...
async function generateInventory() {
  ...
  return providers.map((provider) => {
    ...
    return {
      ...
      status: deriveStatus({ providerId: provider.providerId, lastStatus: providerHealth.lastStatus, configurationRequirement }),
      ...
      authenticationRequirement: configurationRequirement || (KNOWN_LIVE_PROVIDER_IDS.has(provider.providerId) ? "None (public, no-auth source)" : "None currently required (stub — real integration would need credentials)"),
