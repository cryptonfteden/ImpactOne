...
test("running the full orchestrator surfaces the real news report via the standard per-agent result fields", async () => {
  ...
  assert.ok(["fulfilled", "error", "timeout"].includes(newsResult.status), "a real network call may succeed, gracefully degrade, or (rarely) time out in this environment");
