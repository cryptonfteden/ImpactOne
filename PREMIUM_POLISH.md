...
**Phase:** CINEMATIC-EXPERIENCE-001. Companion to [CINEMATIC_GUIDELINES.md](CINEMATIC_GUIDELINES.md). Every item is stated as **current (real, shipped in `306b4a5`/`10a3b73`) → polish (proposed)** — this is a concrete implementation checklist for a future phase, not new invention. Organized by the mission's own 5 Focus Areas.
...
| Current (real) | Polish |
|---|---|
| Ad hoc per-element spacing values across `flagshipScreen.css` | Convert to the `rhythm-*` scale defined in [VISUAL_HIERARCHY.md](VISUAL_HIERARCHY.md) |
| All 10 panels equal visual weight regardless of real content | Apply the 3-tier hierarchy system from [VISUAL_HIERARCHY.md](VISUAL_HIERARCHY.md), driven by each panel's own real `useFlagshipData.js` state |
...
| Connecting lines fixed opacity `0.22` regardless of panel tier | Brightness driven by the panel's real tier (Primary/Secondary/Ambient), per [VISUAL_HIERARCHY.md](VISUAL_HIERARCHY.md)'s reading-order cue |
...
2. The 3-tier hierarchy system + tied motion-amplitude signal ([VISUAL_HIERARCHY.md](VISUAL_HIERARCHY.md), Motion table above) — one real computation driving two real visual outcomes.
