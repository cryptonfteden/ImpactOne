...
**Purpose:** How [AGENT_PLATFORM_ARCHITECTURE.md](AGENT_PLATFORM_ARCHITECTURE.md)'s design behaves at 50+ agents, and what changes (and what deliberately does not) as this platform moves toward distributed, horizontally-scaled execution. No code was written or changed to produce this document.
...
A single, central Scheduler (per [AGENT_PLATFORM_ARCHITECTURE.md](AGENT_PLATFORM_ARCHITECTURE.md)) does not mean a single point of contention if built correctly:
...
The worker-pool concurrency model in [AGENT_PLATFORM_ARCHITECTURE.md](AGENT_PLATFORM_ARCHITECTURE.md) scales linearly with the number of workers available, precisely because agents are stateless and Bus-mediated rather than holding in-process state — any worker can pick up any queued run. This is the single most important precondition for everything else in this document: **an agent that accumulates in-process state between runs cannot be safely run by an arbitrary worker**, which is why statelessness is treated as non-negotiable in the core architecture, not a style preference.
